...
class NetWrapper(torch.nn.Module):
    ...
    def forward(self, batch_data, args, is_cluster, cur_num_mix=2):
        ...
        for n in range(N):
            pred_masks[n] = self.net_maskformer(feat_latent, feat_sound, classes[n], feat_frames[n], motions[n], motion_masks[n], is_cluster)
...
def save_metrics_to_txt(epoch, sdr_mix, sdr, sir, sar, filename="music_ours.txt"):
    # 如果文件不存在，创建并写入表头
    if not os.path.exists(filename):
        with open(filename, 'w') as f:
            f.write("Epoch\tSDR_mixture\tSDR\tSIR\tSAR\n")
    
    # 写入当前epoch的指标
    with open(filename, 'a') as f:
        f.write(f"{epoch}\t{sdr_mix:.4f}\t{sdr:.4f}\t{sir:.4f}\t{sar:.4f}\n")

def evaluate(netWrapper, loader, history, epoch, args, is_cluster, cur_num_mix):
    ...
    for i, batch_data in enumerate(loader):
        # forward pass
        err,_, outputs = netWrapper.forward(batch_data, args, is_cluster, cur_num_mix)
        err = err.mean()
    ...
    # save metrics to a txt file after each batch or epoch
    save_metrics_to_txt(epoch, sdr_mix_meter.average(), sdr_meter.average(), sir_meter.average(), sar_meter.average())
...
# train one epoch
def train(netWrapper, loader, optimizer, history, epoch, mask_optimizer, mask_lr_scheduler, args, is_cluster, cur_num_mix):
    ...
    for i, batch_data in enumerate(loader):
        ...
        err,loss,_= netWrapper.forward(batch_data, args, is_cluster, cur_num_mix)
        err = err.mean()
...
def main(args):
    ...
    if args.mode == 'eval':
        args.testing = True
        evaluate(netWrapper, loader_val, history, 0, args, False, args.num_mix)
    ...
    for epoch in range(1, args.num_epoch + 1):
        ...
        cur_num_mix = num_mix
        train(netWrapper, loader_train, optimizer, history, epoch, mask_optimizer, mask_lr_scheduler, args, is_cluster, cur_num_mix)
        ...
        if epoch % args.eval_epoch == 0:
            args.testing = True
            evaluate(netWrapper, loader_val, history, epoch, args, False, args.num_mix)
            args.testing = False
            # checkpointing
        ...
        if epoch != 70:
            ...
            new_name = f'./av_weight_save_{epoch}'
            rename_folder(save_dir, new_name)
