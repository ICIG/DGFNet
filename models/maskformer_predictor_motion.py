...
def save_av_weight_hook(module, input, output):
    av_weight = output.cpu().detach().numpy()  # 将结果转移到 CPU 并去掉梯度
            
    # 保存目录
    save_dir = './av_weight_save'
    #os.makedirs(save_dir, exist_ok=True)
            
    # 文件名使用计数器（挂载到 save_av_weight_hook 上）
    file_counter = getattr(save_av_weight_hook, 'save_counter', 0)
    save_av_weight_hook.save_counter = file_counter + 1  # 增加计数器
            
    # 文件路径
    file_path = os.path.join(save_dir, f"av_weight_{file_counter}.npy")
            
    # 保存为 .npy 文件
    np.save(file_path, av_weight)
    #print(f"Saved av_weight to {file_path}")
...
class TransformerPredictorMotion(nn.Module):
    ...
    def __init__(self, in_channels, hidden_dim, num_queries, nheads, dropout, dim_feedforward, enc_layers, dec_layers, pre_norm, mask_dim, deep_supervision, enforce_input_project):
        ...
        self.av_fusion = InnerProd(in_channels)
        
        #self.instr_aug_linear = nn.Linear(config.hidden_size,1)
        #self.instr_ori_linear = nn.Linear(config.hidden_size,1)
        # 使用1x1卷积代替线性层
        self.instr_av_conv = nn.Conv2d(in_channels, 1, kernel_size=1)  # 1x1卷积，输出通道为1
        self.instr_ori_conv = nn.Conv2d(in_channels, 1, kernel_size=1)  # 1x1卷积，输出通道为1
        self.instr_sigmoid = nn.Sigmoid()

        # 注册 hook
        self.instr_sigmoid.register_forward_hook(save_av_weight_hook)

       


    def forward(self, x, mask_features, classes, obj_feature, motion_feature, motion_mask, is_cluster):

        #print(f"x shape {x.shape}")
        av_feat = self.av_fusion(obj_feature, x)
        #print(f"av_feat shape {av_feat.shape}")
        # 使用1x1卷积计算门控权重
        av_liner_weight = self.instr_av_conv(av_feat)  # 输出形状 [batch_size, 1, height, width]
        ori_liner_weight = self.instr_ori_conv(x)      # 输出形状 [batch_size, 1, height, width]
        av_weight = self.instr_sigmoid(av_liner_weight+ori_liner_weight)

        #print(av_weight.shape)
        x = torch.mul(av_weight, av_feat) + torch.mul((1-av_weight),x)
        x = av_feat
        ...
        obj_feature = obj_feature.unsqueeze(-1)  # 增加 sequence_length，形状 [batch_size, channels, 1]
        obj_feature = self.upobj(obj_feature)  # 输入到 Conv1d   
        obj_feature = obj_feature.squeeze(-1)    # 去掉序列维度，形状变为 [batch_size, in_channels]
...
class InnerProd(nn.Module):
    def __init__(self, fc_dim):
        ...
        self.conv_img = nn.Conv2d(in_channels=32, out_channels=256, kernel_size=1)

    def forward(self, feat_img, feat_sound):
        (B, C, H, W) = feat_sound.size()
        # 1. 将 feat_img 调整为 [B, C, 1, 1]，使其可以传入 Conv2d 层
        feat_img = feat_img.unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]

        # 2. 通过 1x1 卷积调整视觉特征通道数
        feat_img = self.conv_img(feat_img)  # [B, 256, 1, 1]，卷积后的视觉特征

        # 3. 重新调整 feat_img 的形状为 [B, 256] 以与 feat_sound 进行内积
        feat_img = feat_img.view(B, 256)  # 压平为 [B, 256]，与音频特征进行内积
